...
>(
	...
	try {
		const indexedAt = await retrySearchIndexOperation(async () => {
			await (searchService as CognitiveSearchDomain).createIndexIfNotExists(indexDefinition);
			await (searchService as CognitiveSearchDomain).indexDocument(indexDefinition['name'], document);
			return new Date();
		}, maxAttempts);
...
export async function deleteFromSearchIndexWithRetry(
	...
	await retrySearchIndexOperation(async () => {
		await (searchService as CognitiveSearchDomain).deleteDocument(indexName, { id: documentId });
	}, maxAttempts);
}
