...
export const CallHandleLogout: Story = {
	play: ({ canvasElement }) => {
		...
		type LogoutAuth = Parameters<typeof HandleLogout>[0];
		type LogoutClient = Parameters<typeof HandleLogout>[1];

		const mockAuth = {
			removeUser: () => Promise.resolve(),
			signoutRedirect: () => Promise.resolve(),
		} satisfies Pick<LogoutAuth, 'removeUser' | 'signoutRedirect'>;

		const mockApolloClient = {
			clearStore: () => Promise.resolve([]),
		} satisfies Pick<LogoutClient, 'clearStore'>;

		HandleLogout(
			mockAuth as unknown as LogoutAuth,
			mockApolloClient as unknown as LogoutClient,
