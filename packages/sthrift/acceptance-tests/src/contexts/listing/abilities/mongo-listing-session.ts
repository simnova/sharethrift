import { MongoSession } from '../../../shared/abilities/mongo-session.js';
import type { CreateItemListingInput, ItemListingResponse } from './listing-session.js';

export class MongoListingSession extends MongoSession {
	context = 'listing';
	...
	private serializeInput(input: CreateItemListingInput): Record<string, unknown> {
		return {
			...
			// Support isDraft parameter to control listing state (isDraft: false = Active)
