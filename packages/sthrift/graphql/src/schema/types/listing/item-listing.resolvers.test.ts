...
test.for(feature, ({ Scenario }) => {
    ...
    Scenario('Querying myListingsAll with search and filters', ({ Given, And, When, Then }) => {
        ...
        When('the myListingsAll query is executed', async () => {
            const resolver = itemListingResolvers.Query?.myListingsAll as TestResolver<{
                page: number;
                pageSize: number;
                searchText: string;
        ...
        When('the createItemListing mutation is executed', async () => {
            try {
                const resolver = itemListingResolvers.Mutation
                    ?.createItemListing as TestResolver<{ input: CreateItemListingInput }>;
    ...
    Scenario('Pausing an item listing successfully', ({ Given, And, When, Then }) => {
        Given('a user with a verifiedJwt in their context', () => {
            context = makeMockGraphContext();
        });
        And('a valid listing ID for an active listing', () => {
            ...
            ).mockResolvedValue(createMockListing({ id: 'listing-1', state: 'Paused' }));
        });
    ...
    Scenario('Pausing a non-existent item listing', ({ Given, And, When, Then }) => {
        Given('a user with a verifiedJwt in their context', () => {
            context = makeMockGraphContext();
        });
