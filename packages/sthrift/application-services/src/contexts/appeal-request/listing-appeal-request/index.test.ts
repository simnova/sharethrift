...
import { ListingAppealRequest } from './index.ts';
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	let service: ReturnType<typeof ListingAppealRequest>;
	...
	BeforeEachScenario(() => {
			...
			domainDataSource: {
				AppealRequest: {
					ListingAppealRequest: {
						ListingAppealRequestUnitOfWork: {
		...
		service = ListingAppealRequest(mockDataSources);
	});

	Scenario(
		'Creating a listing appeal request through the application service',
		({
			Given,
			When,
			Then,
			...
			let result: Domain.Contexts.AppealRequest.ListingAppealRequest.ListingAppealRequestEntityReference;

			Given('a listing appeal request application service', () => {
				expect(service).toBeDefined();
			});

			When('I create a listing appeal request', async () => {
	...
	Scenario(
		'Getting a listing appeal request by ID through the application service',
		({
			Given,
			When,
			Then,
			...
			let result: Domain.Contexts.AppealRequest.ListingAppealRequest.ListingAppealRequestEntityReference | null;

			Given('a listing appeal request application service', () => {
				expect(service).toBeDefined();
			});

			When('I get a listing appeal request by id "appeal-1"', async () => {
	...
	Scenario(
		'Getting all listing appeal requests through the application service',
		({
			Given,
			When,
			Then,
			...
			When(
				'I get all listing appeal requests with page 1 and pageSize 10',
				async () => {
	...
	Scenario(
		'Updating a listing appeal request state through the application service',
		({
			Given,
			When,
			Then,
			...
			When(
				'I update listing appeal request "appeal-1" state to "accepted"',
				async () => {
