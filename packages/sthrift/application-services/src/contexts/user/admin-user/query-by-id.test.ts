...
import { type AdminUserQueryByIdCommand, queryById } from './query-by-id.ts';
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
	...
	let command: AdminUserQueryByIdCommand;
	// biome-ignore lint/suspicious/noExplicitAny: Test mock variable
	let result: any;
	...
	Scenario(
		'Successfully retrieving an admin user by ID',
		({ Given, And, When, Then }) => {
	...
	Scenario('Retrieving non-existent admin user', ({ Given, When, Then }) => {
