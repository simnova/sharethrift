...
function makeTestContext(overrides?: {
	...
	const command: ConversationSendMessageCommand = {
		...
		messageContents: ['Hello, I would like to reserve this item.'],
		authorId: 'author-123',
		...overrides?.command,
	};
...
test.for(feature, ({ Scenario }) => {
	...
	Scenario('Successfully sending a message', ({ Given, When, Then, And }) => {
		Given('a valid conversation exists with ID "conv-123"', () => {
			ctx = makeTestContext();
		});

		And(
			'I am a participant in the conversation with author ID "author-123"',
			() => {
		...
		And(
			'the message content is "Hello, I would like to reserve this item."',
			() => {
		...
		When('I send the message', async () => {
			ctx.mockMessagingRepo.sendMessage.mockResolvedValue({
				...
				contents: {
					valueOf: () => ['Hello, I would like to reserve this item.'],
				},
				createdAt: new Date(),
			});
			try {
		...
		And('the message should be returned with the content and author ID', () => {
			...
			expect(result?.contents?.valueOf()).toEqual([
				'Hello, I would like to reserve this item.',
			]);
		});
	});
	...
	Scenario(
		...
		({ Given, When, Then, And }) => {
			Given('a valid conversation exists with ID "conv-123"', () => {
				result = undefined;
				error = undefined;
				ctx = makeTestContext({ command: { messageContents: [''] } });
			});
			...
			Then(
				'an error should be thrown indicating message content cannot be empty',
				() => {
					expect(error).toBeDefined();
					// VOArray throws an array of errors when validation fails
					const errorMessage = Array.isArray(error)
						? (error[0] as Error).message
						: error?.message;
	...
	Scenario(
		'Sending a message that exceeds character limit',
		({ Given, When, Then, And }) => {
			Given('a valid conversation exists with ID "conv-123"', () => {
				result = undefined;
				error = undefined;
				ctx = makeTestContext({
					command: { messageContents: ['a'.repeat(2001)] },
				});
			});
			...
			Then(
				'an error should be thrown indicating message exceeds maximum length',
				() => {
					expect(error).toBeDefined();
					// VOArray throws an array of errors when validation fails
					const errorMessage = Array.isArray(error)
	...
	Scenario(
		'Sending a message to non-existent conversation',
		({ Given, When, Then }) => {
			...
			Then(
				'an error should be thrown indicating conversation not found',
				() => {
					expect(error).toBeDefined();
	...
	Scenario(
		'Sending a message when not a participant',
		({ Given, When, Then, And }) => {
			Given('a valid conversation exists with ID "conv-123"', () => {
				ctx = makeTestContext({
					command: {
			...
			And('I am not a participant in the conversation', () => {
				...
				// Mock messaging repo to reject unauthorized user
	...
	Scenario(
		...
		({ Given, When, Then, And }) => {
			Given('a valid conversation exists with ID "conv-123"', () => {
				ctx = makeTestContext({ command: { messageContents: ['Hello'] } });
			});
	...
	Scenario(
		'Handling messaging service send failure',
		({ Given, When, Then, And }) => {
			Given('a valid conversation exists with ID "conv-123"', () => {
				ctx = makeTestContext({ command: { messageContents: ['Hello'] } });
			});
	...
	Scenario(
		'Sharer can send message in conversation',
		({ Given, When, Then, And }) => {
			Given(
				'a valid conversation exists with sharer ID "sharer-123" and reserver ID "reserver-456"',
				() => {
					result = undefined;
					error = undefined;
					ctx = makeTestContext({
						conversation: {
							sharer: {
								id: 'sharer-123',
							} as unknown as Domain.Contexts.Conversation.Conversation.ConversationEntityReference['sharer'],
							reserver: {
								id: 'reserver-456',
							} as unknown as Domain.Contexts.Conversation.Conversation.ConversationEntityReference['reserver'],
						},
						command: {
							authorId: 'sharer-123',
							messageContents: ['Hello from the sharer!'],
						},
					});
				},
			);
			...
			When('I send a message as the sharer', async () => {
				ctx.mockMessagingRepo.sendMessage.mockResolvedValue({
					...
					contents: { valueOf: () => ['Hello from the sharer!'] },
					createdAt: new Date(),
				});
				try {
					result = await sendMessage(ctx.dataSources)(ctx.command);
			...
			Then(
				'the message should be sent successfully with author ID "sharer-123"',
				() => {
					expect(error).toBeUndefined();
					expect(ctx.mockMessagingRepo.sendMessage).toHaveBeenCalledWith(
						expect.objectContaining({
	...
	Scenario(
		'Reserver can send message in conversation',
		({ Given, When, Then, And }) => {
			Given(
				'a valid conversation exists with sharer ID "sharer-123" and reserver ID "reserver-456"',
				() => {
					...
					ctx = makeTestContext({
						...
						command: {
							...
							messageContents: ['Hello from the reserver!'],
						},
					});
				},
			);
			...
			When('I send a message as the reserver', async () => {
				ctx.mockMessagingRepo.sendMessage.mockResolvedValue({
					...
					contents: { valueOf: () => ['Hello from the reserver!'] },
					createdAt: new Date(),
				});
				try {
					result = await sendMessage(ctx.dataSources)(ctx.command);
			...
			Then(
				'the message should be sent successfully with author ID "reserver-456"',
				() => {
					expect(error).toBeUndefined();
					expect(ctx.mockMessagingRepo.sendMessage).toHaveBeenCalledWith(
						expect.objectContaining({
