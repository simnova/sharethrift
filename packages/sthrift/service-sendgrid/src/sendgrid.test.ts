...
test.for(feature, ({ Scenario }) => {
	...
	Scenario('Constructing SendGrid with missing API key', ({ Given, When, Then }) => {
	...
	Scenario('Constructing SendGrid with valid API key', ({ Given, When, Then, And }) => {
	...
	Scenario('Sending email with magic link in development mode', ({ Given, When, Then, And }) => {
	...
	Scenario('Sending email with magic link in production mode', ({ Given, When, Then, And }) => {
		const userEmail = 'user@example.com';
		const magicLink = 'https://example.com/magic/abc123';
	...
	Scenario('Replacing magic link placeholder in template', ({ Given, When, Then, And }) => {
		const template =
		...
		When('the template is processed', () => {
			...
			result = (sendGridInstance as any).replaceMagicLink(template, magicLink);
		});
	...
	Scenario('Handling invalid email template JSON', ({ Given, When, Then, And }) => {
		const userEmail = 'user@example.com';
		const magicLink = 'https://example.com/magic/abc123';
