...
import {
	getUserAppealRequestReadRepository,
	type UserAppealRequestReadRepository,
} from './user-appeal-request.read-repository.ts';

const test = { for: describeFeature };
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const feature = await loadFeature(
	path.resolve(__dirname, 'features/user-appeal-request.read-repository.feature'),
);
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	let repository: UserAppealRequestReadRepository;

	BeforeEachScenario(() => {
		repository = getUserAppealRequestReadRepository(makeModels(), makePassport());
	});
	...
	Scenario('Getting an appeal request by ID when not found', ({ When, Then }) => {
		...
		When('I call getById with a nonexistent ID', async () => {
			...
			(models.AppealRequest.UserAppealRequest.findOne as ReturnType<typeof vi.fn>).mockReturnValue({
				exec: vi.fn(() => Promise.resolve(null)),
			});
			repository = getUserAppealRequestReadRepository(models, makePassport());
			try {
