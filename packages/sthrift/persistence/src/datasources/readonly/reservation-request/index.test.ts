...
test.for(feature, ({ Scenario }) => {
	Scenario(
		...
		({ Then, And }) => {
			...
			And('ReservationRequestContext should be a function', () => {
				expect(typeof ReservationRequestIndex.ReservationRequestContext).toBe(
					'function',
				);
			});
		},
	...
	Scenario(
		'Creating Reservation Request Read Context',
		({ Given, And, When, Then }) => {
			...
			When('I call ReservationRequestContext with models and passport', () => {
				result = ReservationRequestIndex.ReservationRequestContext(
					mockModels,
					mockPassport,
				);
			});
			...
			Then(
				'it should return an object with ReservationRequest property',
				() => {
