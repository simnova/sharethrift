...
const feature = await loadFeature(
	path.resolve(__dirname, 'features/admin-user.uow.feature'),
);
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
	let adminUserModel: Models.User.AdminUserModelType;
	...
	Scenario('Creating an AdminUser Unit of Work', ({ When, Then, And }) => {
		When(
			'I call getAdminUserUnitOfWork with the AdminUser model and passport',
			() => {
				result = getAdminUserUnitOfWork(adminUserModel, passport);
			},
		);
