...
test.for(feature, ({ Scenario }) => {
	Scenario(
		'System passport for user should use permission function',
		({ Given, When, Then }) => {
	...
	Scenario(
		'System user passport should extend SystemPassportBase',
		({ Given, When, Then }) => {
			let passport: SystemUserPassport;
	...
	Scenario(
		'System passport forAdminUser should return visa with permission function',
		({ Given, When, Then }) => {
			let passport: SystemUserPassport;
			// biome-ignore lint/suspicious/noExplicitAny: Test mock
			let visa: any;
	...
	Scenario(
		'System passport forPersonalUser visa should evaluate permission correctly',
		({ Given, When, Then }) => {
			let passport: SystemUserPassport;
			// biome-ignore lint/suspicious/noExplicitAny: Test mock
			let visa: any;
			...
			When('I request access to a personal user and check permissions', () => {
				const mockUser = {
					id: 'test-personal-user-id',
				} as PersonalUserEntityReference;
				visa = passport.forPersonalUser(mockUser);
			});

			Then(
				'the permission function should be called with permissions object',
				() => {
					// biome-ignore lint/suspicious/noExplicitAny: Test mock
