...
test.for(feature, ({ Scenario }) => {
	Scenario(
		'System passport for account plan should use permission function',
		({ Given, When, Then }) => {
			...
			When('I request access to an account plan', () => {
				const mockAccountPlan = {
					id: 'test-account-plan-id',
				} as AccountPlanEntityReference;
	...
	Scenario(
		'System account plan passport should extend SystemPassportBase',
		({ Given, When, Then }) => {
			let passport: SystemAccountPlanPassport;
