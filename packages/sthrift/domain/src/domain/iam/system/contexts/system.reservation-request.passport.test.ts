...
const feature = await loadFeature(
	path.resolve(
		__dirname,
		'features/system.reservation-request.passport.feature',
	),
);

test.for(feature, ({ Scenario }) => {
	Scenario(
		'System passport for reservation request should use permission function',
		({ Given, When, Then }) => {
	...
	Scenario(
		'System reservation request passport should extend SystemPassportBase',
		({ Given, When, Then }) => {
			let passport: SystemReservationRequestPassport;
