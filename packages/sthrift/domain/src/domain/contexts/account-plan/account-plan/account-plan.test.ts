...
const feature = await loadFeature(
  path.resolve(__dirname, 'features/account-plan.feature'),
);

function makePassport(permissions: Partial<{ canCreateAccountPlan: boolean; canUpdateAccountPlan: boolean }> = {}) {
  return vi.mocked({
    accountPlan: {
      forAccountPlan: vi.fn(() => ({
        determineIf: (fn: (p: { canCreateAccountPlan: boolean; canUpdateAccountPlan: boolean }) => boolean) =>
          fn({
...
function makeBaseProps(overrides: Partial<AccountPlanProps> = {}): AccountPlanProps {
  return {
    id: 'plan-1',
    name: 'Pro',
...
test.for(feature, ({ Scenario, Background, BeforeEachScenario }) => {
  ...
  Scenario('Creating a new account plan instance', ({ When, Then, And }) => {
    When('I create a new AccountPlan aggregate using getNewInstance', () => {
      newPlan = AccountPlan.getNewInstance(
        makeBaseProps(),
        passport,
        {
          name: 'Pro',
  ...
  Scenario('Updating name without permission throws', ({ Given, When, Then }) => {
    Given('a plan with no update permission', () => {
      passport = makePassport({ canCreateAccountPlan: false, canUpdateAccountPlan: false });
