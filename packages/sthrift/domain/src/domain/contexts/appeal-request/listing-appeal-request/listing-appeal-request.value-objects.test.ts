...
const feature = await loadFeature(
	path.resolve(__dirname, 'features/listing-appeal-request.value-objects.feature'),
);
...
test.for(feature, ({ Scenario }) => {
	...
	Scenario('Creating a Reason with valid value', ({ When, Then }) => {
		let value: string;
		When('I create a Reason with "This listing was incorrectly blocked and should be restored"', () => {
			value = new ValueObjects.Reason('This listing was incorrectly blocked and should be restored').valueOf();
		});
		Then('the value should be "This listing was incorrectly blocked and should be restored"', () => {
			expect(value).toBe('This listing was incorrectly blocked and should be restored');
		});
	});

	Scenario('Creating a Reason with leading and trailing whitespace', ({ When, Then }) => {
		let value: string;
		When('I create a Reason with "  This listing was incorrectly blocked  "', () => {
			value = new ValueObjects.Reason('  This listing was incorrectly blocked  ').valueOf();
		});
		Then('the value should be "This listing was incorrectly blocked"', () => {
			expect(value).toBe('This listing was incorrectly blocked');
		});
	});
