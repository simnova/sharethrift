...
test.for(feature, ({ Scenario }) => {
	Scenario(
		'Profile value object can be created with valid props',
		({ Given, When, Then }) => {
	...
	Scenario(
		'Profile firstName getter returns correct value',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
			let value: string;
	...
	Scenario(
		'Profile lastName getter returns correct value',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
			let value: string;
	...
	Scenario(
		'Profile aboutMe getter returns correct value',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
			let value: string;
	...
	Scenario(
		'Profile location getter returns Location value object',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
			...
			Then(
				'it should return a PersonalUserAccountProfileLocation instance',
				() => {
	...
	Scenario(
		'Profile billing getter returns Billing value object',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
			...
			Then(
				'it should return a PersonalUserAccountProfileBilling instance',
				() => {
	...
	Scenario(
		'Profile firstName setter requires valid visa',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
	...
	Scenario(
		'Profile firstName setter works with valid visa',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
	...
	Scenario(
		'Profile allows setters when entity is new',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;
	...
	Scenario(
		'Profile aboutMe setter works with valid visa',
		({ Given, When, Then }) => {
			let profile: PersonalUserProfile;

			Given(
				'I have a profile instance with a permissive visa and aboutMe',
				() => {
					const props = makeProfileProps({ aboutMe: 'Old bio' });
					const visa = createMockVisa(true) as UserVisa;
					const root = createMockRoot(false) as PersonalUserAggregateRoot;
					profile = new PersonalUserProfile(props, visa, root);
				},
			...
			When('I set the aboutMe property', () => {
				profile.aboutMe = 'Updated bio';
			});

			Then('the aboutMe should be updated', () => {
				expect(profile.aboutMe).toBe('Updated bio');
			});
		},
	);
});
