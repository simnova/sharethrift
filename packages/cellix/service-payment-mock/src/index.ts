...
export class ServicePaymentMock
	implements ServiceBase<ServicePaymentMock>, PaymentService
{
	...
	public startUp(): Promise<
		Exclude<ServicePaymentMock, ServiceBase<ServicePaymentMock>>
	> {
		if (this.http) {
			throw new Error('ServicePaymentMock is already started');
		}

		this.http = axios.create({ baseURL: this.mockBaseUrl });
		console.log(`ServicePaymentMock started in MOCK mode: ${this.mockBaseUrl}`);
		return Promise.resolve(this as Exclude<
			ServicePaymentMock,
			ServiceBase<ServicePaymentMock>
		>);
	}

	public shutDown(): Promise<void> {
		if (!this.http) {
			throw new Error('ServicePaymentMock is not started - shutdown cannot proceed');
		}
		this.http = undefined;
		console.log('ServicePaymentMock stopped');
