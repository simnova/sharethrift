...
import { ServicePaymentMock } from './index.ts';
...
test.for(feature, ({ Scenario, BeforeEachScenario }) => {
  let service: ServicePaymentMock;
  let error: unknown;
  let result: unknown;

  BeforeEachScenario(() => {
    service = undefined as unknown as ServicePaymentMock;
    error = undefined;
    result = undefined;
  });

  Scenario('Instantiating with baseUrl', ({ Given, When, Then, And }) => {
    Given('a ServicePaymentMock with baseUrl "http://localhost:3001"', () => {
    });
    When('I instantiate the service', () => {
      service = new ServicePaymentMock('http://localhost:3001');
    });
    Then('the service should be an instance of ServicePaymentMock', () => {
      expect(service).toBeInstanceOf(ServicePaymentMock);
    });
  ...
  Scenario('Getting service getter without starting throws', ({ Given, When, Then }) => {
    Given('a ServicePaymentMock', () => {
      service = new ServicePaymentMock('http://localhost:3001');
    });
  ...
  Scenario('Shutting down without starting throws', ({ Given, When, Then }) => {
    Given('a ServicePaymentMock', () => {
      service = new ServicePaymentMock('http://localhost:3001');
    });
