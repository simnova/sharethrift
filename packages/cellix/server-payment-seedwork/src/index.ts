...
export function startMockPaymentServer(config: PaymentConfig): Promise<void> {
			...
			ctx: [
				{
					...
					data: {
						...
						targetOrigins: [config.frontendBaseUrl],
						mfOrigin: config.paymentBaseUrl,
					},
				},
			],
		};
	...
	app.post('/payments/v1/authorizations', (req: Request, res: Response) => {
		res.json({
			status: 'AUTHORIZED',
	...
	app.get('/microform/bundle/:version/iframe.html', (_req: Request, res: Response) => {
...
<html>
	...
	app.post(
		'/pts/v2/customers',
		(
			req: Request<Record<string, never>, PaymentTransactionResponse, { customerProfile: CustomerProfile; paymentTokenInfo: PaymentTokenInfo }>,
	...
	app.post(
		...
		(
			req: Request<{ customerId: string }, PaymentTransactionResponse, { customerProfile: CustomerProfile; paymentTokenInfo: PaymentTokenInfo }>,
			res: Response<PaymentTransactionResponse>,
		) => {
			const { customerId } = req.params;
			const { customerProfile, paymentTokenInfo } = req.body;
	...
	app.get(
		...
		(
			...
			try {
				...
				res.status(500).json({ error: 'Internal mock error' } as unknown as CustomerPaymentInstrumentResponse);
			}
		},
	);
	...
	app.get(
		'/tms/v2/customers/:customerId/payment-instruments',
		(
			req: Request<{ customerId: string }, CustomerPaymentInstrumentsResponse>,
	...
	app.post(
		'/rbs/v1/plans',
		(
			_req: Request<Record<string, never>, PlanCreationResponse, { plan: PlanCreation }>,
	...
	app.post(
		...
		(
			_req: Request<Record<string, never>, SubscriptionResponse, { subscription: Subscription }>,
