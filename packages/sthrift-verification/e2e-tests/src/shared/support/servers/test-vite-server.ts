...
export class TestViteServer extends PortlessServer {
	...
	protected get spawnArgs() {
		return ['--force', 'sharethrift.localhost', 'pnpm', 'exec', 'vite'];
	}
	protected get cwd() {
	...
	protected override get extraEnv() {
		const oauthAuthority = buildUrl(
			'mock-auth.sharethrift.localhost',
			'/user-portal',
		);
		const adminOauthAuthority = buildUrl(
			'mock-auth.sharethrift.localhost',
			'/admin-portal',
		);
		...
		return {
			...
			VITE_B2C_ADMIN_AUTHORITY: adminOauthAuthority,
