...
import { type E2ELoginPage, type E2EOnboardingPage, LoginPage, OnboardingPage } from '@sthrift-verification/verification-shared/pages';
...
function loadTestCredentials(): { username: string; password: string } {
	...
	return {
		username: process.env['E2E_USERNAME'] || defaults['E2E_USERNAME'] || 'alice@example.com',
...
export async function performOAuth2Login(page: Page, options: OAuth2LoginOptions = {}): Promise<void> {
	...
	try {
		...
		console.error(`[oauth2-login] Failed to load login page.\n` + `  URL: ${url}\n` + `  HTML (first 2000 chars): ${content.slice(0, 2000)}`);
		throw gotoError;
	}
	...
	// An already-onboarded user transiently lands on /signup after the OAuth
	// redirect: the client-side onboarding redirect only sends them home once
	// the current-user query resolves. In login mode, wait for that redirect to
	// settle so we don't mistake the transient /signup for a genuine signup.
	if (mode !== 'signup') {
		await page
			.waitForURL((url) => !url.pathname.includes('/signup'), {
				timeout: 15_000,
			})
			.catch(() => undefined);
	}
