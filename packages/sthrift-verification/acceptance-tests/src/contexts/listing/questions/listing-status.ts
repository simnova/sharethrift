...
export class ListingStatus extends Question<Promise<string>> {
	...
	private readStatusFromDomain(actor: AnswersQuestions & UsesAbilities): string | undefined {
		try {
			return CreateListingAbility.as(actor as unknown as Actor).getCreatedListing()?.state;
		} catch {
			return undefined;
		}
	}
