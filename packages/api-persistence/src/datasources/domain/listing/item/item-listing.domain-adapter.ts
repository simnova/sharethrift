...
export class ItemListingDomainAdapter implements ItemListingProps {
  public doc: Models.Listing.ItemListing;

  constructor(doc: Models.Listing.ItemListing) {
    this.doc = doc;
  }
  ...
  set state(value: Domain.Contexts.ListingState) {
    this.doc.state = value.valueOf() as NonNullable<Models.Listing.ItemListing['state']>;
  }
  ...
  set sharingHistory(value: string[]) {
    ...
    this.doc.sharingHistory = value as unknown as Models.Listing.ItemListing['sharingHistory'] || [];
  }
...
export class ItemListingConverter {
  toDomain(
    doc: Models.Listing.ItemListing,
    passport: Passport
