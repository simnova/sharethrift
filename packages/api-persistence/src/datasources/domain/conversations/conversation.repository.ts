...
export class ConversationRepository extends MongooseSeedwork.MongoRepositoryBase<
...
> implements Domain.Contexts.Conversation.ConversationRepository<PropType> {
  async getById(id: string): Promise<Domain.Contexts.Conversation.Conversation<PropType> | null> {
  ...
  async getByListingAndParticipants(listing: string, sharer: string, reserver: string): Promise<Domain.Contexts.Conversation.Conversation<PropType> | null> {
    const mongoConversation = await this.model.findOne({ listing, sharer, reserver }).exec();
    if (!mongoConversation) return null;
    return this.typeConverter.toDomain(mongoConversation, this.passport);
  }

  async getUserConversations(userId: string): Promise<Domain.Contexts.Conversation.Conversation<PropType>[]> {
    const mongoConversations = await this.model.find({ $or: [{ sharer: userId }, { reserver: userId }] }).exec();
    return mongoConversations.map(doc => this.typeConverter.toDomain(doc, this.passport));
  }
