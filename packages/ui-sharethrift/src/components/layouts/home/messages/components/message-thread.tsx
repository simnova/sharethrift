...
export function MessageThread({ messages, loading, error, messageText, setMessageText, sendingMessage, handleSendMessage, messagesEndRef, currentUserId, contentContainerStyle }: MessageThreadProps) {
  if (loading) {
      ...
      <div style={{ display: 'flex', flexDirection: isOwn ? 'row-reverse' : 'row', gap: 8, maxWidth: 400 }}>
        ...
        <div
          style={{
            ...
            color: isOwn ? 'var(--color-highlight)' : 'var(--color-message-text)',
            borderRadius: 16,
            padding: '8px 16px',
