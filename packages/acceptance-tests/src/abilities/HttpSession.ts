...
/**
 * HttpSession - makes real HTTP calls to the GraphQL API.
 *
 * Following Screenplay.js design recommendations:
 * "The HttpSession is where you encapsulate all of the fetch, WebSocket and EventSource logic.
 *  This is the class your UI will use in production. You will also use it in tests."
 *
 * Benefits:
 * - Tests full HTTP + Domain layer coverage
 ...
 */
export class HttpSession extends Ability implements Session {
	 ...
	 */
	static at(apiUrl: string): HttpSession {
		return new HttpSession(apiUrl);
	}

	/**
	 * Create a listing via GraphQL HTTP call
	 */
	...
	/**
	 * Get listing by ID via GraphQL HTTP call
	 */
	...
	/**
	 * Execute GraphQL HTTP request using fetch
	 */
	 ...
	 */
	static as(actor: Actor): HttpSession {
		return actor.abilityTo(HttpSession);
	}
}
