...
- Backend permission enforcement for blob uploads is **strong by design**. Before a client is permitted to upload, it must request a server-signed authorization and supply the intended upload metadata (MIME type, file size, and optionally filename). The server can reject this request outright if the metadata fails validation. If the server approves the request, the signed headers cryptographically bind the upload to the exact approved parameters — the specific `Content-Type` (MIME type), exact `Content-Length` (file size, confirmed within a permitted maximum), and precise blob path (storage location). Any deviation from these approved values by the client will invalidate the signature and the upload will be rejected by Azure Blob Storage. Of the direct-to-storage mechanisms we evaluated for this ADR, none offer equivalent per-request upload constraints.
- Future improvements will focus on expanding domain-driven permission checks [per the DDD security model] before server-signed request headers are issued, and on exploring pre-upload scanning alternatives to further reduce the post-upload malware exposure window.