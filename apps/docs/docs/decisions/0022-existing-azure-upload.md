...
- **Option 2: Shared Key SAS tokens (current approach)**
...
Chosen option: **Option 2: Shared Key SAS tokens (current approach)**, because SAS URLs are generated by the backend per request with the correct blob path, metadata, and tags, and are cryptographically signed so that clients or third parties cannot tamper with or escalate their permissions.
