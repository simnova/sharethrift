...
- **Option B: Shared Key SAS tokens (current approach)**
    - A Shared Key–signed Shared Access Signature (SAS) is a time‑limited, permission‑scoped URL generated using the storage account’s access key. It allows the client to upload or modify blobs temporarily without exposing the actual account key, and the backend can control exactly which permissions, metadata, and expiry apply for each individual upload request.

## Authorization Decision Outcome

Chosen option: **Option 2: Option B: Shared Key SAS tokens (current approach)**, because SAS URLs are generated by the backend per request with the correct blob path, metadata, and tags, and are cryptographically signed so that clients or third parties cannot tamper with or escalate their permissions.
