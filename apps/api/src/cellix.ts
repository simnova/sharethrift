...
> {
	...
	/**
	 * Registers an Azure Function Timer trigger endpoint.
	 *
	 * @remarks
	 * The `handlerCreator` is invoked per timer execution and receives the application services host.
	 * Use it to create scheduled tasks (e.g., cleanup jobs, data retention enforcement).
	 * Registration is allowed in phases `'app-services'` and `'handlers'`.
	 *
	 * @param name - Function name to bind in Azure Functions.
	 ...
	 * @example
	 * ```ts
	 * registerAzureFunctionTimerHandler('cleanupExpiredConversations', '0 0 2 * * *', (host) => {
	 *   return async (timer, ctx) => {
	 *     const app = await host.forRequest();
	 *     await app.Cleanup.processExpiredConversations();
	 *   };
	 * });
	 * ```
	 */
		...
		StartedApplication<ContextType>
{
	...
	private readonly pendingTimerHandlers: Array<
		PendingTimerHandler<AppServices>
	> = [];
		...
		for (const h of this.pendingHandlers) {
			app.http(h.name, {
				...
				handler: (request, context) => {
					...
					return h.handlerCreator(this.appServicesHostInternal)(
						request,
						context,
					);
				},
