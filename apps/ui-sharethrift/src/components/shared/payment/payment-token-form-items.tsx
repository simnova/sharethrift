...
export const PaymentTokenFormItems: FC<PaymentTokenFormItemsProps> = (props) => {
  ...
  const createFlexObj = useCallback(
    // biome-ignore lint/suspicious/noExplicitAny: <explanation>
    (keyId: string, clearValidationText: (data: any, field: string) => void): void => {
      try {
        // biome-ignore lint/suspicious/noExplicitAny: <explanation>
        ...
        <Col xs={24} sm={12}>
          <Form.Item<PaymentTokenFormFieldType>
            ...
            rules={[
              {
                required: true,
                message: "Please enter the expiration date as shown on the card.",
              },
            ]}
          >
            {/** biome-ignore lint/nursery/useUniqueElementIds: <explanation> */}
            <DatePicker.MonthPicker
              ...
              disabledDate={(current: any) => {
                return current && current < dayjs().utc().endOf("day").subtract(1, "day");
              }}
              className="w-full"
            />
          </Form.Item>
