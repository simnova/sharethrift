	...
	return {
		hookSpecificOutput: {
			...
			additionalContext: [
				...
				"5. You CANNOT stop until the Synthesizer writes and publishes the audit.",
				...
				"- `DependencySecurity` MUST apply safe mechanical fixes for obsolete waivers and mechanical patched-version override updates, and MUST record every changed path in `appliedFixes` in its report.",
				"- `CodeQuality` and `Performance` are report-only and MUST NOT modify repo files.",
				"- The subagent-stop hook verifies every expected report is present before allowing the Synthesizer to spawn.",
				"- After the Synthesizer writes the audit markdown, it MUST create a branch, commit the audit artifact plus any `appliedFixes`, push, and open a PR.",
